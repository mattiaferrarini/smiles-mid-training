...
def prepare_datasets(config):
    ...
    sciq_path = os.path.expandvars(
        config.get("data", {}).get("sciq_path", "sciq.jsonl")
    )
    ...
    metamathqa_path = os.path.expandvars(
        config.get("data", {}).get("metamathqa_path", "metamathqa.jsonl")
    )
...
def train(config, model, tokenizer, dataset_splits, base_config):
    ...
    if local_rank == 0:
        ...
        final_dir = os.path.join(
            os.path.expandvars(config["training"]["output_dir"]), subdir
        )
