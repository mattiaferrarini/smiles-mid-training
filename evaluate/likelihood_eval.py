...
def get_likelihood(
    model, tokenizer, question, answer, debug=False, use_smiles_annotation=False
):
    """
    ...
    if debug:
        print(
            f"  Debug - Full text length: {input_ids.shape[1]}, Prompt length: {prompt_len}, Answer: '{answer}'"
        )
    ...
    neg_log_likelihood = loss_fct(
        answer_logits.view(-1, answer_logits.size(-1)), answer_labels.view(-1)
    )
    ...
    for idx in tqdm(range(len(ds)), desc=f"{config}"):
        ...
        for example in examples_list:
            ...
            for candidate in candidates:
                score = get_likelihood(
                    model,
                    tokenizer,
                    question,
                    candidate,
                    debug=debug,
                    use_smiles_annotation=use_smiles_annotation,
                )
                if debug:
...
def eval_path(
    model_path, tokenizer, local_rank, debug=False, use_smiles_annotation=False
):
    """
...
def log_path_results(eval_results):
    ...
    for res in results:
        print(
            f"Config: {res['config']}, Accuracy: {res['accuracy']:.2%} ({res['correct']}/{res['total']})"
        )
    print("=" * 20)
...
def log_all_results(all_results):
    ...
    print("\n" + "=" * 50)
    print(
        f"BEST MODEL (Overall Accuracy): {os.path.basename(os.path.normpath(best_result['path']))}"
    )
    print("=" * 50)
    ...
    if os.path.exists(model_path) and os.path.isdir(model_path):
        ...
        if os.path.exists(os.path.join(model_path, "config.json")):
            ...
            training_config_path = os.path.join(
                os.path.dirname(model_path), "training_config.yaml"
            )
        else:
...
def run_likelihood_eval(model_path, output_dir, debug=False, annotate_smiles=False):
    ...
    my_checkpoints = [
        cp for i, cp in enumerate(checkpoint_paths) if i % world_size == rank
    ]
    ...
    for checkpoint_path in my_checkpoints:
        ...
        eval_results = eval_path(
            ...
            local_rank,
            debug=debug,
            use_smiles_annotation=annotate_smiles,
        )
