...
def load_training_dataset(config, tokenizer):
    ...
    # --- MODIFICA PER PROFILING: Taglio del dataset ---
    LOGGER.warning("DEBUG MODE: Using a small subset of data for profiling (max 1000 samples)!")
    if len(dataset) > 1000:
...
def train(
    ...
    # Training arguments
    # --- MODIFICA: Forziamo report_to="none" se use_wandb è False ---
...
def run_training_pipeline(config, hf_token, embedding_override = None):
    ...
    if accelerator.is_main_process:
        LOGGER.info("=" * 80)
        LOGGER.info("UNIFIED TRAINING PIPELINE (COPY - PROFILING MODE)")
        LOGGER.info("=" * 80)
    ...
    # --- MODIFICA: Disabilitiamo WandB per questa copia ---
    use_wandb = False
    wandb_run_name = None
    wandb_config = None
    # ------------------------------------------------------
