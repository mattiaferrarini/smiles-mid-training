...
def main():
    ...
    NEW_DATASET_OUTPUT_PATH = config.get(
        "new_dataset_output_path", "processed_dataset_output"
    )
    ...
    ORIGINAL_ANNOTATED_FIELD = config.get(
        "original_annotated_field", "text_annotated_v1tags"
    )
    ORIGINAL_SMILES_COUNT_FIELD = config.get(
        "original_smiles_count_field", "text_annotated_v1_smiles_count"
    )
    ...
    try:
        ...
        for i, example in enumerate(shuffled_dataset):
            ...
            new_text, my_smiles_count = annotate_smiles(
                original_text, USE_LLM=USE_LLM, model=model if USE_LLM else None
            )
            ...
            my_smiles = set(
                s.strip()
                for s in re.findall(r"\[START_SMILES\](.*?)\[END_SMILES\]", new_text)
            )
            ...
            if COMPARE_TO_GT:
                gt_smiles = set(
                    s.strip()
                    for s in re.findall(
                        r"\[START_SMILES\](.*?)\[END_SMILES\]", annotated_text
                    )
                )
            ...
            if not WHOLE_DATASET:
                ...
                if COMPARE_TO_GT:
                    logging.info(
                        f"Original Smiles Count (Variable): {annotated_smiles_count}"
                    )
                logging.info(f"My Regex Smiles Count: {my_smiles_count}")
                if COMPARE_TO_GT:
                    logging.info(
                        f"False Positives (My Regex found, but not in GT): {list(fp_set)}"
                    )
                    logging.info(
                        f"False Negatives (In GT, but My Regex missed): {list(fn_set)}"
                    )
        ...
        if SAVE_NEW_DATASET and count > 0:
            print(
                f"Saving new dataset with {len(new_data_buffer)} rows to {NEW_DATASET_OUTPUT_PATH}..."
            )
            try:
        ...
        if COMPARE_TO_GT:
            logging.info("=" * 50)
            logging.info(
                "TOP 1000 MOST FREQUENT FALSE POSITIVES (Predicted but NOT in GT):"
            )
            ...
            logging.info("=" * 50)
            logging.info(
                "TOP 1000 MOST FREQUENT FALSE NEGATIVES (In GT but MISSED by Regex):"
            )
            logging.info("Count\tString")
